...
# def test_calib(net, test_loader):
...
#
#         logits = net(data) # 2 * data - 1) @ mughees because the test data is already normalized
#
#         # accuracy
#         pred = logits.data.max(1)[1]
#         num_correct += pred.eq(target.data).sum().item()
#
#         out = F.softmax(logits, dim=1).max(1)[0]
#         confidence.extend(out.data       .to('cpu').numpy().squeeze().tolist())  # i think np.stack can be used
#         correct   .extend(pred.eq(target).to('cpu').numpy().squeeze().tolist())
#
#     # acc, test_confidence, test_correct
