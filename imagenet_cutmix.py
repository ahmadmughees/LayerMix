#  python imagenet.py --num-classes 1000 --mixing-set /media/ifiveo/fast/datasets/deviantart_1 --pretrained --batch-size 512 --workers 16 --save checkpoints/imagenet/resnet50_bs_512_w_16_epochs_90 --epochs 90 --arch resnet50
import argparse
import os
import random
import shutil
...
from imagent_utils import imagenet_r_wnids, imagenet_a_wnids, all_classes
...
def main():
    ...
    if args.evaluate:
        ...
        if evaluate_c:
            evaluate_variant(model, normalize,
    ...
    blending_fns = {
        ...
        "ladder_mix_3_steps": [add, multiply, add, multiply, random_pixels, random_elems],
        "ladder_mix_4_steps": [add, multiply, add, multiply, random_pixels, random_elems],
    ...
    train_dataset = MixerDataset(
        ...
        depth           = args.depth,
        width           = args.width,
        image_aug_fns   = augs_image,  #+ augs_c_test,
        ...
        jsd             = args.jsd,
        normalize       = normalize)  # TODO make sure you verify the arguments you are passing.
    ...
    # cutmix = v2.CutMix(num_classes=len(classes_chosen), alpha=0.2)
    mixup = v2.MixUp(num_classes=len(classes_chosen), alpha=0.2)
    ...
    evaluate_variant(model, normalize, args.batch_size, args.workers, args.imagenet_c_dir, args.save, variant="c")
    evaluate_variant(model, normalize, args.batch_size, args.workers, args.imagenet_c_bar_dir, args.save, variant="c_bar")


def evaluate_variant(model, normalize, batch_size, workers, c_dir, save_path, variant):
...
def validate(val_loader, model, criterion, args, r=False, a=False, adv=None):
    ...
    # todo this is super interesting as i cant see which function actually is being used lol.
    ...
    print(f"RMS {100 * calib_err(np.array(confidence.copy()), np.array(correct.copy()), p='2'):.3f}")
